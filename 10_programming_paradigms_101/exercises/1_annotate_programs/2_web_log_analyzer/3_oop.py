...
class WebLogAnalyzer:
    ...
    def count_server_errors(self):
        self.error_count = sum(1 for entry in self.log_entries if entry['status_code'].startswith('5'))

    def analyze_traffic(self):
        for entry in self.log_entries:
            endpoint = entry['endpoint']
            self.traffic_by_endpoint[endpoint] = self.traffic_by_endpoint.get(endpoint, 0) + 1
    ...
    def print_analysis(self):
        ...
        print("\nTraffic by Endpoint:")
        for endpoint, count in sorted(self.traffic_by_endpoint.items(), key=lambda x: x[1], reverse=True):
